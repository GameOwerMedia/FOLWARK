...
export const chapter=(w:World)=>w.day<4?['I','Ziemia po ludziach']:w.day<7?['II','Ciezar obietnic']:['III','Kto przezyje zime'];
export function nextObjective(w:World){
 if(w.scenario!=='survival')return 'Rozwoj folwarku / '+w.season;
 if(w.day>=9&&w.regime.heating!=='high')return 'Silny mroz: wlacz mocne ogrzewanie. Zwykle juz nie wystarczy.';
 if(w.resources.wood<40)return 'Pilne: zdobadz opal. Kwatery zuzywaja drewno.';
...
export function survivalPanel(w:World){
 ...
 return '<section class="survival-panel"><span class="eyebrow">SCENARIUSZ / DLA DOROSLYCH</span><h2>Wolnosc ma swoja cene</h2><p>Ludzie odeszli. Zwierzeta przejely ziemie, lecz zima nie uznaje zwyciestw. Rada decyduje, kto pracuje, kto je i co wolno pamietac.</p><div class="mission-order"><strong>ROZKAZ RADY</strong><p>'+nextObjective(w)+'</p></div><h3>Warunki przetrwania</h3><ul><li>Przetrwaj 10 dni. Mroz zaczyna sie w dniu 7.</li><li>Ocal co najmniej 8 mieszkancow i utrzymaj 40% ciepla o swicie dnia 11.</li><li>Mniej niz 6 zywych lub ponad 80% niepokojow konczy rzady rady.</li></ul><p>Glod i zimno zabijaja. Przymus oslabia ciala. Strach moze uciszyc protest, ale nie naprawia krzywdy. Zakonczenie oceni takze uczciwosc twoich rzadow.</p>'+meter('Cieplo kwater',r.heat)+meter('Zaufanie',r.trust)+meter('Strach',fear)+meter('Uczciwosc rady',r.integrity)+'<h3>Ogrzewanie / '+temperature(w)+' C</h3><label class="regime-select">Spalanie drewna<select data-heating aria-label="Ogrzewanie">'+(['off','normal','high']as const).map(k=>'<option value="'+k+'" '+(r.heating===k?'selected':'')+'>'+({off:'Wylaczone / 0',normal:'Zwykle / '+Math.round(fuelPerDay(w,'normal'))+' na dzien',high:'Mocne / '+Math.round(fuelPerDay(w,'high'))+' na dzien'})[k]+'</option>').join('')+'</select></label><p>Mroz zwieksza zuzycie opalu. Od dnia 9 potrzebne jest mocne ogrzewanie. Lecznica dziala tylko przy cieple i zapasach jedzenia.</p><button class="wide-action" data-action="insulate" '+(r.insulated||!w.canPay({wood:60,stone:20})?'disabled':'')+'>'+icon(r.insulated?'check':'house')+(r.insulated?'Kwatery ocieplone / -30% opalu':'Ociepl kwatery / 60 drewna, 20 kamienia')+'</button><div class="casualties"><span>Zgony: '+r.deaths.length+'</span><span>Oddani ludziom: '+r.sold.length+'</span></div></section>';
}
